...
$GetCateGories = $CommonController->getCategoriesBySection("movies");
$newCategoryIDArray = [];
...
if (isset($_GET["cate"]) && $_GET["cate"] != "") {
    ...
    $QueryData = ["request" => "Get", "table" => "webtvtheme_banners", "data" => ["portalurl" => $PortalLink, "type" => "movies", "category" => $categoryid], "extra" => ["ORDER BY id DESC"]];
    $BannerData = $funconn->WebTVClient_ExecuteQuery($QueryData, $conn);
    if (empty($BannerData)) {
        $QueryData = ["request" => "Get", "table" => "webtvtheme_banners", "data" => ["portalurl" => $PortalLink, "type" => "movies", "category" => "all"], "extra" => ["ORDER BY id DESC"]];
        $BannerData = $funconn->WebTVClient_ExecuteQuery($QueryData, $conn);
    }
    $portallink = $_SESSION["webTvplayer"]["portallink"];
    ...
    $getBlockedCategoriesIts = $CommonController->getBlockedCategoriesIts($conn, $getActivePortalID, "movies");
    $getBlockedStreamsIts = $CommonController->getBlockedStreamsIts($conn, $getActivePortalID, "movies", $categoryid);
    $variablesArray = ["activepage" => "movies", "pagetitle" => "Movies", "classsname" => "main-bg", "section" => "movies", "logovalue" => $LogoIs, "sitetitle" => $sitetitle, "funconn" => $funconn, "sortby" => $sortby, "categoryid" => $categoryid, "getBlockedStreamsIts" => $getBlockedStreamsIts, "getBlockedCategoriesIts" => $getBlockedCategoriesIts, "Getblockedsection" => $Getblockedsection, "BannerData" => $BannerData, "categories" => $FinalCategoriesArray, "license" => $ValidLicense];
    $dispatcher->dispatch("header", $conn, $variablesArray);
    $dispatcher->dispatch("navigation", $conn, $variablesArray);
    $dispatcher->dispatch("moviesview", $conn, $variablesArray);
