...
$GetCateGories = $CommonController->getCategoriesBySection("catchup");
$GetFavforCategories = $funconn->webtvpanel_getFavforCategories("catchup", $conn);
$newCategoryIDArray = [];
...
$getBlockedCategoriesIts = $CommonController->getBlockedCategoriesIts($conn, $getActivePortalID, "catchup");
if (!empty($GetCateGories) && $GetCateGories["result"] == "success") {
    foreach ($GetCateGories["data"] as $StreamsData) {
    ...
    if (!empty($FirstCategoriesIDS)) {
        ...
        if ($GetOnlyCateGories["result"] == "success" && !empty($GetOnlyCateGories["data"])) {
            ...
            foreach ($GetOnlyCateGories["data"] as $CatKey) {
                if (!in_array($CatKey->category_id, $getBlockedCategoriesIts) && array_key_exists($CatKey->category_id, $FirstCategoriesIDS)) {
...
if (isset($_GET["c"]) && $_GET["c"] != "") {
    ...
    $variablesArray = ["activepage" => "catchup", "pagetitle" => "Catchup", "classsname" => "main-bg", "section" => "catchup", "logovalue" => $LogoIs, "funconn" => $funconn, "sitetitle" => $sitetitle, "getBlockedCategoriesIts" => $getBlockedCategoriesIts, "Getblockedsection" => $Getblockedsection, "CurrentCateName" => $CurrentCateName, "categoryid" => $categoryid, "categories" => $FinalCategoriesArray, "license" => $ValidLicense];
    $dispatcher->dispatch("header", $conn, $variablesArray);
    $dispatcher->dispatch("navigation", $conn, $variablesArray);
    $dispatcher->dispatch("catchuplist", $conn, $variablesArray);
    $dispatcher->dispatch("footer", $conn, $variablesArray);
} else {
    echo "<script>window.location.href = 'catchup.php';</script>";
    exit;
}
