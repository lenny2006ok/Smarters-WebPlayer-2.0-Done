...
if (isset($_GET["c"]) && $_GET["c"] != "") {
    ...
    if (isset($FinalCategoriesArray["data"]) && !empty($FinalCategoriesArray["data"])) {
        foreach ($FinalCategoriesArray["data"] as $Cateval) {
            if ($categoryid == $Cateval->category_id) {
                $CurrentCateName = $Cateval->category_name;
            }
        }
    }
    $variablesArray = ["activepage" => "live", "pagetitle" => "Live", "classsname" => "main-bg", "section" => "live", "funconn" => $funconn, "logovalue" => $LogoIs, "sitetitle" => $sitetitle, "categoryid" => $categoryid, "getBlockedCategoriesIts" => $getBlockedCategoriesIts, "Getblockedsection" => $Getblockedsection, "CurrentCateName" => $CurrentCateName, "categories" => $FinalCategoriesArray, "license" => $ValidLicense];
    $dispatcher->dispatch("header", $conn, $variablesArray);
    $dispatcher->dispatch("navigation", $conn, $variablesArray);
    $dispatcher->dispatch("livelist", $conn, $variablesArray);
