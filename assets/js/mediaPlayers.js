function setPlayer($movieVideoLink)
{
	...
	else
	{
	   	...
	   	if($(document).find('.PlayerHolder').data('jwplayer') == 'fail')
	   	{
	   		set_flowplayer($movieVideoLink);
	   	}
	   	else if($(document).find('.PlayerHolder').data('flowplayer') == 'fail')
	   	{
...
{
        $(document).find('#player-holder').removeClass('d-none');
        $(document).find('.poster-details').addClass('d-none');
...
flowPlayerH.on("error", function (e, api, err) {
  ...
  if (err.code === 2 || err.code === 4) {
   $(document).find('.PlayerHolder').data('flowplayer','fail');
          ...
          $(document).find('.player_changeIssue').html('Flow Player cannot play this Video Format.This Video Format is not supported by browser yet!');
          $(document).find('.player_changeIssue').animate({'top':'10px'});
           setTimeout (function(){
             $(document).find('.player_changeIssue').animate({'top':'-300px'});
           }, 3000);  
