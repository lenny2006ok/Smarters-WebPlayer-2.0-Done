...
$GetCateGories = $CommonController->getCategoriesBySection("series");
$newCategoryIDArray = [];
...
if (isset($_GET["cate"]) && $_GET["cate"] != "") {
    ...
    $QueryData = ["request" => "Get", "table" => "webtvtheme_banners", "data" => ["portalurl" => $PortalLink, "type" => "series", "category" => $categoryid], "extra" => ["ORDER BY id DESC"]];
    $BannerData = $funconn->WebTVClient_ExecuteQuery($QueryData, $conn);
    if (empty($BannerData)) {
        $QueryData = ["request" => "Get", "table" => "webtvtheme_banners", "data" => ["portalurl" => $PortalLink, "type" => "series", "category" => "all"], "extra" => ["ORDER BY id DESC"]];
        $BannerData = $funconn->WebTVClient_ExecuteQuery($QueryData, $conn);
    }
    $portallink = $_SESSION["webTvplayer"]["portallink"];
    ...
    $getBlockedCategoriesIts = $CommonController->getBlockedCategoriesIts($conn, $getActivePortalID, "series");
    $getBlockedStreamsIts = $CommonController->getBlockedStreamsIts($conn, $getActivePortalID, "series", $categoryid);
    $variablesArray = ["activepage" => "series", "pagetitle" => "Series", "classsname" => "main-bg", "section" => "series", "logovalue" => $LogoIs, "funconn" => $funconn, "sitetitle" => $sitetitle, "getBlockedStreamsIts" => $getBlockedStreamsIts, "getBlockedCategoriesIts" => $getBlockedCategoriesIts, "Getblockedsection" => $Getblockedsection, "sortby" => $sortby, "categoryid" => $categoryid, "BannerData" => $BannerData, "categories" => $FinalCategoriesArray, "license" => $ValidLicense];
    $dispatcher->dispatch("header", $conn, $variablesArray);
    $dispatcher->dispatch("navigation", $conn, $variablesArray);
    $dispatcher->dispatch("seriesview", $conn, $variablesArray);
