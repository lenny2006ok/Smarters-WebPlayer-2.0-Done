...
$GetFavforCategories = $funconn->webtvpanel_getFavforCategories("movies", $conn);
if (!empty($GetCateGories) && $GetCateGories["result"] == "success") {
    $newCategoryIDArray["all"] = (int) ["category_id" => "all", "category_name" => "All", "parent_id" => "0"];
...
$variablesArray = ["activepage" => "categories", "pagetitle" => "Movies Categories", "classsname" => "dash-bg", "section" => "movies", "logovalue" => $LogoIs, "sitetitle" => $sitetitle, "Getblockedsection" => $Getblockedsection, "categories" => $FinalCategoriesArray, "license" => $ValidLicense];
$dispatcher->dispatch("header", $conn, $variablesArray);
$dispatcher->dispatch("navigation", $conn, $variablesArray);
$dispatcher->dispatch("movies", $conn, $variablesArray);
