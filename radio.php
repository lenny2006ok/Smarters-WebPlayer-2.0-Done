...
$GetFavforCategories = $funconn->webtvpanel_getFavforCategories("radio", $conn);
...
if (!empty($FirstCategoriesIDS)) {
    ...
    if ($GetOnlyCateGories["result"] == "success" && !empty($GetOnlyCateGories["data"])) {
        ...
        $FinalCategoriesArray = $GetOnlyCateGories;
...
$variablesArray = ["activepage" => "categories", "pagetitle" => "Radio Categories", "classsname" => "dash-bg", "section" => "radio", "logovalue" => $LogoIs, "sitetitle" => $sitetitle, "categoryid" => $categoryid, "Getblockedsection" => $Getblockedsection, "categories" => $FinalCategoriesArray, "license" => $ValidLicense];
$dispatcher->dispatch("header", $conn, $variablesArray);
$dispatcher->dispatch("navigation", $conn, $variablesArray);
$dispatcher->dispatch("radio", $conn, $variablesArray);
