...
if (isset($_GET["c"]) && $_GET["c"] != "") {
    ...
    $variablesArray = ["activepage" => "live", "pagetitle" => "Live", "classsname" => "main-bg", "section" => "live", "funconn" => $funconn, "logovalue" => $LogoIs, "sitetitle" => $sitetitle, "getBlockedCategoriesIts" => $getBlockedCategoriesIts, "Getblockedsection" => $Getblockedsection, "categoryid" => $categoryid, "CurrentCateName" => $CurrentCateName, "categories" => $FinalCategoriesArray, "license" => $ValidLicense];
    $dispatcher->dispatch("header", $conn, $variablesArray);
    $dispatcher->dispatch("navigation", $conn, $variablesArray);
    $dispatcher->dispatch("epglist", $conn, $variablesArray);
